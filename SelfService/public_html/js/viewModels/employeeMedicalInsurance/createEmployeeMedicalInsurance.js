define(['ojs/ojcore', 'knockout', 'jquery', 'appController', 'util/commonhelper', 'config/services', 'ojs/ojtrain', 'ojs/ojbutton', 'ojs/ojinputtext', 'ojs/ojknockout-validation', 'ojs/ojselectcombobox', 'ojs/ojdatetimepicker', 'ojs/ojdialog', 'ojs/ojtimezonedata', 'ojs/ojinputnumber', 'ojs/ojpopup', 'ojs/ojcheckboxset', 'ojs/ojradioset', 'ojs/ojlabel', 'ojs/ojgauge'], function (oj, ko, $, app, commonUtil, services) {

    function createEmployeeMedicalInsurance() {
        var self = this;

        //********   rootViewModel.globalPersonFuseModel.gradeAboveP1(rootViewModel.personDetails().grade()); ********//
        ...
        self.employeeMedicalInsuranceModel = {
            personName : ko.observable(""), passengerName1 : ko.observable(""), passengerName2 : ko.observable(""), passengerName3 : ko.observable(""), passengerName4 : ko.observable(""), requestDate : self.currentDate(), requestType : ko.observable(""), reason : ko.observable(""), otherReasons : ko.observable(""), dependent_1_details : ko.observable(""), dependent_2_details : ko.observable(""), dependent_3_details : ko.observable(""), dependent_4_details : ko.observable(""), dependent_5_details : ko.observable(""), employeeGrade : ko.observable(""), IdIqamaNumber : ko.observable(""), employeBirthDate : ko.observable(""), remarks : ko.observable(""), imageBase64 : ko.observable(""), status : ko.observable(""), name : ko.observable(""), createdBy : rootViewModel.personDetails().personNumber(), creationDate : ko.observable(""), personNumber : ko.observable(""), personId : ko.observable(""), managerId : ko.observable(""), IS_DRAFT : ko.observable("")
        };
        ...
        var getAdvancedAnnualLeaveReportFn = function (data) {
            if (data) {
                ...
                if (jsonData.length != 0) {
                    //                    self.advancedAnnualLeaveModel.leave(jsonData.LEAVE_TYPE);
        ...
        function addEmployeeMedicalInsurance() {
            if (self.clickedButton() != event.currentTarget.id) {
                self.clickedButton(event.currentTarget.id);
                self.employeeMedicalInsuranceModel.personNumber(self.specialistSummary() === 'true' ? rootViewModel.specialistSelectedEmployee().PersonNumber : rootViewModel.personDetails().personNumber());
                self.employeeMedicalInsuranceModel.managerId(self.specialistSummary() === 'true' ? rootViewModel.specialistSelectedEmployee().ManagerId : rootViewModel.personDetails().managerId());
                self.employeeMedicalInsuranceModel.dependent_1_details(self.employeeMedicalInsuranceModel.dependent_1_details().toString());
                self.employeeMedicalInsuranceModel.dependent_2_details(self.employeeMedicalInsuranceModel.dependent_2_details().toString());
                self.employeeMedicalInsuranceModel.dependent_3_details(self.employeeMedicalInsuranceModel.dependent_3_details().toString());
                self.employeeMedicalInsuranceModel.dependent_4_details(self.employeeMedicalInsuranceModel.dependent_4_details().toString());
                self.employeeMedicalInsuranceModel.dependent_5_details(self.employeeMedicalInsuranceModel.dependent_5_details().toString());
                self.employeeMedicalInsuranceModel.reason(self.employeeMedicalInsuranceModel.reason().toString());
                self.employeeMedicalInsuranceModel.requestType(self.employeeMedicalInsuranceModel.requestType().toString());

                var jsonData = ko.toJSON(self.employeeMedicalInsuranceModel);
                var addEmployeeMedicalInsuranceFn = function (data1) {
                ...
                services.addEmployeeMedicalInsurance(jsonData).then(addEmployeeMedicalInsuranceFn, app.failCbFn);
            }
        }
        ...
        this.submitDraft = function () {
            self.employeeMedicalInsuranceModel.IS_DRAFT("YES");
            self.employeeMedicalInsuranceModel.status("DRAFT");
            document.querySelector("#yesNoDialog").open();
        };
            ...
            var getApprovalList = function (data) {
                ...
                $.each(data.items, function (i, item) {
                    self.approvaldataTB2.push( {
                        name : "TRF", type : item.notification_type, status : item.role_type
    ...
    return new createEmployeeMedicalInsurance();
});